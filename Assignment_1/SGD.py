...
def trainModel(lossType=None):
    ...
    for i in range(700):
        ...
        for step in range(7):
            ...
            _, err, currentW, currentb, predictions = sess.run(
                [train, loss, W, b, y_predicted], feed_dict={X: batch_data, y_target: batch_labels})
            # if step % 10 == 0:
...
def classify_result(Data, Target, W, b, LossType):
    N = Target.shape[0]
    predict = 0
    classifier = np.zeros((N, 1))
    if (LossType == "MSE"):
        predict = (tf.matmul(Data, W) + b)
    elif (LossType == "CE"):
...
if __name__ == '__main__':
    W, b, predictions = trainModel("MSE")
    trainData, validData, testData, trainTarget, validTarget, testTarget = loadData()
    ...
    #classifier,loss,acc = classify_result(trainData, trainTarget, W,b,"CE")
    #print ("done")
    #print (loss)
    #print (acc)
    '''
